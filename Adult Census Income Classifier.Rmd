...
**Step 2: Seperate into training, testing and validation sets.** As it is rather computationally expensive to use the standard 80:20 or 70:30 training to validation ratio, only 10,000 sample rows are used for training, with 5000 allotted for training purposes and the approximately 30,000 remaining rows allotted for the validation set which will not be utilized until final testing.
...
#Seperating it into train and test
...
The Naive Bayes algorithm is also employed using a random search consisting of 10 iterations and with 5-fold cross validation repeated three times. The number of iterations is quite low but as it is very computationally expensive to run a higher number of iterations, only 10 are run.
  
```{r}

set.seed(1, sample.kind = 'Rounding')
control <- trainControl(method = "repeatedcv", number = 5, repeats = 3, search = 'random')
nb_fit <- train(frm, train_set, method="nb", trControl=control, tuneLength = 10)
...
svm_linear_fit2 <- train(frm, data = train_set, method = "svmLinear", trControl=control,  tuneGrid = data.frame(C = seq(9,25,2)))
...
_Svm_Radial_ has several parameters and so random search is employed with 10 iterations, a low number but unavoidable given the computational expense. One could also utilize the bayesian optimization method and use the provided code in the R file.
...
svm_radial_fit <- train(frm, data=train_set, method="svmRadial", trControl=control, tuneLength = 10)
...
plot(SS(rf_fit), print.thres.best.method="closest.topleft", col= '7')
lines(SS(glm_fit), print.thres.best.method="closest.topleft", col='5')
lines(SS(bayesglm_fit), print.thres.best.method="closest.topleft", col= '2')
lines(SS(knn_fit), print.thres.best.method="closest.topleft", col= '3')
lines(SS(svm_linear_fit), print.thres.best.method="closest.topleft", col= '6')
...
plot(PR(rf_fit), add = T, col="8")
plot(PR(glm_fit), add = T, col= '5')
plot(PR(bayesglm_fit), add = T, col= '2')
plot(PR(knn_fit), add = T, col= '3')
plot(PR(svm_linear_fit), add = T, col= '6')
plot(PR(svm_radial_fit), add = T, col= '12')
...
## Final model and test with Validation
